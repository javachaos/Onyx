...
public class InputEvent {
    ...
    public static final short SW_RFKILL_ALL = 0x03;/*
                                                    * rfkill master switch, type
                                                    * "any" set = radio enabled
                                                    */
    ...
    public InputEvent(final long timeSec, final long timeMicroSec, final short type, final short code, final int value,
            final String source) {
     ...
     * @throws IOException
     */
    public static InputEvent parse(final ShortBuffer shortBuffer, final String source) throws IOException {
     ...
     * @throws IOException
     */
    public static InputEvent parse(final ShortBuffer shortBuffer, final String source, final String arch)
            throws IOException {
        short a, b, c, d;
        a = shortBuffer.get();
        b = shortBuffer.get();
        ...
        if (arch.equals("arm")) {
            ...
            c = shortBuffer.get();
            d = shortBuffer.get();
            // fixme : longs are signed, so this is probably not doing what we
            // expect it to do
            timeMicroSec = (d << 48) | (c << 32) | (b << 16) | a;
        }
        final short type = shortBuffer.get();
        final short code = shortBuffer.get();
        c = shortBuffer.get();
        d = shortBuffer.get();
        final int value = (d << 16) | c;
    ...
    public String toString() {
        ...
        return String.format("Event: time %d.%06d, type %d, code %d, value %02x", timeSec, timeMicroSec, type, code,
                value);
    }
